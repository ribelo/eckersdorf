...
(def ^:private all-prop-keys
  [:size :justify-content :align-items :align-self
...
(defn gap [{:keys [size width height]}]
  (let [this (r/current-component)
    ...
    (into [:div (r/merge-props {:style (-> {}
                                           (assoc-some :width (or width size)
...
(defn line [{:keys [size color]
  ...
  (let [this (r/current-component)
        props (apply dissoc (r/props this) all-prop-keys)
        children (r/children this)]
    (into [:div (r/merge-props {:style (flex {:background-color color} size)}
                               props)]
          children)))


(defn scroller [{:keys [size justify-content align-items align-self gap
                        min-width width max-width
                        min-height height max-height
...
(defn hbox [{:keys [size justify-content align-items align-self gap
                    min-width width max-width
                    min-height height max-height]
             :or   {justify-content :start align-items :stretch size :none}}]
  (let [this (r/current-component)
        props (apply dissoc (r/props this) all-prop-keys)
...
(defn vbox [{:keys [size justify-content align-items align-self gap
                    min-width width max-width
                    min-height height max-height]
             :or   {justify-content :start align-items :stretch size :none}}]
  (let [this (r/current-component)
...
(defn box [{:keys [size justify-content align-items align-self gap
                   min-width width max-width
                   min-height height max-height]
            :or   {justify-content :start align-items :stretch size :none}}]
  (let [this (r/current-component)
